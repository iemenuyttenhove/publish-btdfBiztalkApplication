...
function Publish-BTDFBiztalkApplication() {
    ...
    Param(
        ...
        #When set to true, a special environment, named Octopus, is expected in SettingsFileGenerator.xml, and values will be merged with Octopus environment variables.
        [boolean]$mergeOctopusEnvironmentVariablesWithBtdfSettings = $false
    )
    ...
    try {
        ...
        deploy-btdfBiztalkApp -btdfProductName $btdfProductName -isLastBiztalkServer $ImportIntoBiztalkMgmtDb -msbuildExePath $msbuildPath -deployOptionsNameValuePairs $deployOptions -mergeOctopusEnvironmentVariablesWithBtdfSettings $mergeOctopusEnvironmentVariablesWithBtdfSettings
...
function deploy-btdfBiztalkApp() {
    ...
    try {
        ...
        if ($true -eq $mergeOctopusEnvironmentVariablesWithBtdfSettings) {
            if ($null -eq $deployOptionsNameValuePairs) {
                $deployOptionsNameValuePairs = [hashtable]
            }
            #Cannot merge settings this way, because the Octopus_settings.xml file is created as part of the MSBuild script.
            #Merge-OctopusEnvironmentVariablesWithBtdfSettings $installStartInDir $deployOptionsNameValuePairs
            # TODO: Test if it is possible to pass environment settings as the argument named PropsFromEnvSettings
            $propsFromEnvSettings = $addtionalDeployOptions
            $addtionalDeployOptions = "$addtionalDeployOptions /p:PropsFromEnvSettings=$($propsFromEnvSettings)"
        }
        Write-Verbose "Additional Deploy Options: $addtionalDeployOptions"
...
function Merge-OctopusEnvironmentVariablesWithBtdfSettings() {
    param(
        [Parameter(Mandatory = $True)]
        [string]$projectFolder,
        [Parameter(Mandatory = $True)]
        [hashtable]$deployOptionsNameValuePairs
    )
    Write-Host "Merging Octopus environment variables with BTDF settings for $projectFolder"
    $envSettingsDirectory = (Get-Item $projectFolder)
    $octopusSettingsFile = Get-Item (Join-Path(Join-Path $envSettingsDirectory.FullName "EnvironmentSettings") "Octopus_settings.xml")

    if ($True -ne $octopusSettingsFile.Exists) {
        Write-Error "File not found $($octopusSettingsFile.FullName). Add Octopus environment to SettingsFileGenerator.xml and rebuild to have this file generated by BTDF."
    }

    #Parse XML file and look for matching keys in Octopus variables to replace XML values.
    [xml]$settingsXml = Get-Content $octopusSettingsFile.FullName

    if ($null -ne $deployOptionsNameValuePairs) {
        foreach($option in $deployOptionsNameValuePairs.GetEnumerator()) {
            $xpath = "//settings/property[@name='$($option.Name)']"
            $matchingSetting = $settingsXml.SelectSingleNode($xpath)

            if ($null -ne $matchingSetting) {
                Write-Verbose "Found matching setting $($option.Name): $matchingSetting"
                $matchingSetting.InnerText = $($option.Value)
            }
        }

        $settingsXml.Save($octopusSettingsFile.FullName)
    }

    $deployOptionsNameValuePairs["/p:ENV_SETTINGS"] = $octopusSettingsFile.FullName
    Write-Host "Will use $($deployOptionsNameValuePairs["/p:ENV_SETTINGS"]) for environment settings."
}
